...
describe Domoticz::Timer do
  ...
  describe '#apply_to?' do
    ...
    FLAGS = { monday: 0x01, tuesday: 0x02, wednesday: 0x04, thursday: 0x08, friday: 0x10, saturday: 0x20, sunday: 0x40 }.freeze
    ...
    it 'can be applied with a Date object' do
      ...
      expect(subject.apply_to?(Date.new(2016, 0o2, 26))).to be false # it's friday
      expect(subject.apply_to?(DateTime.new(2016, 0o2, 27, 10, 45))).to be true # it's saturday
    end
  end
  describe '#date_array' do
    before(:each) { subject.data = { 'Date' => '2016-02-21' } }
    it 'returns an array: [year, month, day]' do
      expect(subject.date_array).to eq([2016, 0o2, 21])
    end
  end
  ...
  describe '#next_date' do
    let(:now) { DateTime.new(2016, 0o2, 22, 8, 30) }
    context 'when the timer is configured with a fixed date' do
      ...
      context 'when the fixed date is in the future' do
        ...
        it 'returns the fixed date' do
          expect(subject.next_date(now)).to eq(DateTime.new(2050, 0o2, 15, 8, 30))
        end
      end
    ...
    context 'when the timer is configured with "on time"' do
      ...
      it 'returns the next date' do
        expect(subject.next_date(now)).to eq(
          DateTime.new(2016, 0o2, 27, 8, 45)
        )
      end
      it 'the next date should be greater than current date' do
        expect(subject.next_date(DateTime.new(2016, 0o2, 27, 8, 45))).to eq(
          DateTime.new(2016, 0o2, 27, 8, 45).next_day(7)
        )
      end
    end
    context 'when the timer is configured with sunset/sunrise' do
      ...
      context 'when its related to sunrise' do
        ...
        it 'returns the sunrise on the current day when before the sunrise' do
          expect(subject.next_date(DateTime.new(2016, 0o2, 22, 7, 59))).to eq(
            DateTime.new(2016, 0o2, 22, 8, 0)
          )
        end
        it 'returns the sunrize the next day when after the sunrise' do
          expect(subject.next_date(DateTime.new(2016, 0o2, 22, 8, 0))).to eq(
            DateTime.new(2016, 0o2, 23, 8, 0)
          )
        end
      end
      context 'when its related to sunset' do
        ...
        it 'returns the sunset on the current day when before the sunset' do
          expect(subject.next_date(DateTime.new(2016, 0o2, 22, 19, 59))).to eq(
            DateTime.new(2016, 0o2, 22, 20, 0)
          )
        end
        it 'returns the sunset on the next day when after the sunset' do
          expect(subject.next_date(DateTime.new(2016, 0o2, 22, 20, 0))).to eq(
            DateTime.new(2016, 0o2, 23, 20, 0)
          )
        end
      end
    end
